...
// Helper chuyển items sang PayPal
...
exports.create = async (req, res) => {
    try {
        ...
        if (!items?.length) return res.status(400).json({ success: false, message: "Không có tour nào trong booking" });
        ...
        await BookingItem.bulkCreate(items.map(i => ({
...
exports.capture = async (req, res) => {
    try {
        ...
        const qrData = `http://${localIP}:5173/checkin/${booking.id}`;
